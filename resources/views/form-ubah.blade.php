      ...
      <div class="card">
          ...
          <div class="card-body">
            <a  href="{{route ('tambah-buku.index')}}" class="btn btn-primary btn-lg active" role="button" aria-pressed="true">Kembali </a>
              <br><br>
             <form action="{{route ('tambah-buku.update', $tambahbuku->id)}}" method="POST">
             @csrf
             @method('PUT')
             <ul class="list-group">
                 Judul Buku <input type="text" name="inputJudul" required value="{{$tambahbuku->judul_buku}}">
                 Genre <input type="text" name="inputGenre" required value="{{$tambahbuku->genre}}">
                 Tahun Terbit <input type="text" name="inputTahun" required value="{{$tambahbuku->tahun_terbit}}">
             </ul>
             <br>
